...
| `copy.js` | Every line the game **says** that is not part of a level: the mechanic cards, the Holy Talismans' names and lines, and the headings and bylines on a fall, a pause and a clear. Same dual-format arrangement as `levels.js`, same reasons, and edited on the editor's **Copy** tab. Chapter verses are *not* here — they belong to their level. |
...
| `coin-lab.html` | The golden coin, drawn flat like the coin icon: float, rock, shadow and its inscribed panel. Its spark is the game's standard one and is drawn only by the game (`COIN_SPARK`). **Copy COIN block**. |
...
| `monster-lab.html` | **How a swimmer swims** — the stroke every rowing monster takes, in the runner's own beats: a **swim** carrying all of its travel, a **rest**, and a **tension** where the oars reach forward and the body trembles (`SWIM`). Iframe pattern: a board of swimmers — rails both ways, the circler, a hunter and a patrol's crew — pushed live through `__TR.swim()` and `__TR.monsters()`, with a beat lamp beside it that flashes on the runner's beat and a bar showing where the stroke is, so you can see the two land together. Sliders for the three parts, the ease, the body's size, its squash and stretch, the tremble and the oars, plus a pace slider and **spaces per stroke** — a stroke longer than a beat means a swimmer crosses its tile every other one. **Copy SWIM block**. Must be *served*. |
...
| `cosmos-lab.html` | The **sky the whole game hangs in** — the near-black quilt, the nebulae pieced out of that quilt's own squares, two star fields at different depths, and the things that fall through it. Iframe pattern: it pushes `COSMOS` into the running game through `__TR.cosmos()`, so what you are tuning is the shipped sky and not a copy. Sliders for the hue walk and how dark it is, the clouds and their own parallax, star clustering, the second field's share of the near one's movement, and the shooting stars — plus **One now**, which moves the fall clock's origin to this instant rather than faking one. Six presets, and a button for each of the two screens the sky reads differently on. **Copy COSMOS block**. Must be *served*. |
| `footer-lab.html` | The **footer capsule** on the Tileverse — the bar the three rooms off that screen live in. Iframe pattern: it taps its way to the map, then pushes `FOOTER` into the running game through `__TR.footer()`, which writes the block into `--f-*` custom properties the sheet reads. Colour wells and sliders for the two-layer ground, the backdrop blur, the white lip, the shape, the item sizes and the lit tab's glow, six presets, and three buttons that walk you onto each of the three screens the bar appears on. **Copy FOOTER block**. Must be *served*. |
| `swagger-lab.html` | The **swaggers** — the way he runs. Iframe pattern: it lists every swagger the game ships (`__TR.swaggers()`), fits the one you pick on him WITHOUT wearing or saving it (`__TR.fitSwagger()`, which every runner reads through `lookItem`), and patches it live (`__TR.swagger(id, patch)` — a null field falls back to the stock stride). A slider per field — lean, bob, swing, lift, hop, hover, spin, the leap's count/height/hold/split/spin, liftLead, the toe — each with a tick to give the swagger its own value or leave it to `RUNNER`; a pose picker; and the **folded poses' shapes** (`POSES` — lotus, fetal and the opening's splay, in leg bones) through `__TR.poses()`. A big close-up of him running on a tile in his track, drawn by the game into the lab's canvas (`__TR.runnerFigure()`), beside the title screen with him running on it. **Copy SWAGGERS block**, **Copy this line** and **Copy POSES block**. Must be *served*. |
